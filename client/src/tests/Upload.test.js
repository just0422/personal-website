...
describe('Upload test', () => {
  ...
  const projectsFail = () =>
    mock.onGet(projectsUrl).reply(404, 'No skills available');
  const projectsSucceed = () => mock.onGet(projectsUrl).reply(200, projects);
  const projectScreenshotPostFail = () =>
    mock.onPost(projectScreenshotUrl).reply(500, "Couldn't create");
  const projectScreenshotPostSucceed = () =>
    mock.onPost(projectScreenshotUrl).reply(201, screenshots[0]);
  ...
  it('should correctly handle an invalide filetype image select', () => {
    ...
    expect(component.state('screenshotErrorMessage')).toMatch(
      'Invalid file type',
    );
  });
  ...
  it('should correctly handle a too large image select', () => {
    ...
    const screenshot = component.find('.screenshot-file-select');
    screenshot.simulate('change', {
      target: {files: [{type: 'image/jpg', size: 100000000}]},
    });
    component.update();
  ...
  it('should correctly handle a image select', () => {
    ...
    const screenshot = component.find('.screenshot-file-select');
    screenshot.simulate('change', {
      target: {files: [{type: 'image/jpg', size: 1000000}]},
    });
    component.update();
